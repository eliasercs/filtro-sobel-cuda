...
\textbf{Perfilado con Nsight Compute:} la herramienta \texttt{ncu} (Nsight Compute 2026.2.0) se ejecut\'o exitosamente en ambos entornos con los flags \texttt{--set full --target-processes all}. Se perfil\'o cada uno de los 4 kernels de la pipeline sobre \texttt{medium} ($k=5$, $s=0.5$), realizando 45 pases por kernel para garantizar precisi\'on estad\'istica en las m\'etricas. Los reportes generados (\texttt{ncu\_medium\_cuda.ncu-rep}) tienen un tama\~no de 87.2 MB (local, RTX 3060) y 84 MB (servidor, RTX 4090), conteniendo m\'etricas detalladas de occupancy, throughput de memoria, utilizaci\'on de registros y ancho de banda para cada kernel. Estos reportes pueden inspeccionarse con la GUI de Nsight Compute para un an\'alisis granular de cada etapa.
