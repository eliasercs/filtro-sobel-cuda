...
El proyecto incluye adem\'as los siguientes entregables auxiliares:

\begin{itemize}
  \item \texttt{scripts/generate\_test\_images.py} --- genera im\'agenes sint\'eticas para los 4 tama\~nos en caso de que falten las originales.
  \item \texttt{scripts/analyze\_csv.py} --- produce tablas resumen de promedios y speed-ups desde el CSV de resultados.
  \item \texttt{scripts/build\_pdf.py} --- convierte \texttt{docs/informe.md} a PDF usando la biblioteca \texttt{reportlab}.
  \item \texttt{release.bat} --- empaqueta el repositorio completo en un archivo ZIP de entrega.
\end{itemize}
